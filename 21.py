...
def nearestMultiple(num, multiple):
    if num >= multiple:
        near = num +(multiple -num % multiple)
    else:
        near = multiple
    return  near
...
def checkConsecutive(digitsLst):

    for i in range(1,len(digitsLst)):
        if (digitsLst[i] - digitsLst[i-1]) != 1:
            return  False
    return  True
